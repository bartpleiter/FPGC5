...
# script for compiling a C program for BDOS, and upload it to the FPGC4 over the network, whithout running it
...
then
    ...
    then
            ...
            # convert list to binary files and upload to FPGC4

            # WSL1/linux version
            (cd ../Programmer && bash compileROM.sh noPadding && cp code.bin $2 && echo "Uploading $2 to FPGC4 over Network" && python3 netUpload.py $2 && rm $2)
