...
int SHELL_commandIdx = 0; // index in current command
int SHELL_promptCursorPos = 0;
...
int SHELL_numberOfArguments(char* p)
{
    int args = 0;
...
void SHELL_ldir(char* arg)
{
    ...
    // add arg to path
    if (FS_changePath(arg) == ANSW_USB_INT_SUCCESS)
    {
        // do listdir
        ...
        if (FS_open() == ANSW_USB_INT_SUCCESS)
        {
            ...
            if (FS_setCursor(0) == ANSW_USB_INT_SUCCESS)
            {
                ...
                while (bytesSent != fileSize)
                {
                    ...
                    if (partToSend > SHELL_FILE_READ_CHUNK_SIZE)
                    partToSend = SHELL_FILE_READ_CHUNK_SIZE;

                    // read from usb to buffer in byte mode
                    if (FS_readFile(b, partToSend, 0) != ANSW_USB_INT_SUCCESS)
                        uprintln("W: Error reading file\n");
